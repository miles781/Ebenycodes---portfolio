...
const Work = (isDarkMode) => {
  ...
  className='grid grid-cols-auto my-10 gap-5 dark:text-black'> 
    {workData.map((project, index)=>(
        ...
        <div> 
          ...
          <div className='border rounded-full border-black w-6
          aspect-square flex items-center justify-center 
          shadow-[2px_2px_0_#000] group-hover:bg-lime-300 transition
          text-sm'>
           <Image src={ isDarkMode ? assets.right_arrow_bold_dark : assets.send_icon} alt='send icon'
            className='w-5'/> 
          </div>   
        </div> 
  ...
  text-gray-700 border-[0.5px] border-gray-700 rounded-full py-3 px-10 mx-auto dark:text-white dark:border-white dark:hover:bg-darkhover '>
    Show more 
    <Image src={isDarkMode ? assets.right_arrow : assets.right_arrow_bold} alt='Right arrow' />
  </motion.a>
  </motion.div> 
  )
}
